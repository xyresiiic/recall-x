...
import { analyzePerformance, recommendContent, findContentGaps } from "@/lib/analysis";
...
export const Route = createFileRoute("/app")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Memory-powered marketing strategist that learns from your past content.",
      },
    ],
  }),
  component: Dashboard,
    ...
    <section className="relative overflow-hidden rounded-2xl border border-border bg-card p-8">
      ...
      <div className="relative max-w-3xl">
        <div className="inline-flex items-center gap-2 rounded-full border border-border bg-secondary/50 px-3 py-1 text-xs font-mono uppercase tracking-wider text-muted-foreground">
          <span className="h-1.5 w-1.5 rounded-full bg-memory animate-pulse" /> Hindsight Memory ·
          Live
        </div>
        ...
        <p className="mt-5 max-w-2xl text-base text-muted-foreground">
          Every post you've shipped becomes a memory. The agent reads that memory, finds the
          patterns, and tells you what to publish next — with the numbers to prove it.
        </p>
      </div>
    </section>
  );
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="mt-3 flex items-end gap-3">
        <div className="font-display text-6xl text-memory tabular-nums">
          {isLoading ? "—" : count}
        </div>
...
function SeedAndClear() {
  ...
  const seed = useMutation({
    mutationFn: () => seedFn(),
    onSuccess: (r) => {
      toast.success(`Loaded ${r.inserted} demo memories`);
  ...
  return (
    <div className="mt-4 flex gap-2">
      ...
      <Button
        ...
        variant="ghost"
        onClick={() => {
          if (confirm("Clear all memory?")) clear.mutate();
        }}
    ...
    >
      ...
      <div>
        ...
        <Input
          ...
          placeholder="Top 5 AI Marketing Tools"
        />
      </div>
...
function CsvUploadCard() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const items: ContentInputT[] = lines
        .map((line) => {
    ...
    <div className="flex flex-col rounded-xl border border-border bg-card p-5 min-h-[280px]">
      ...
      {count !== null && (
        <div className="mt-1 text-[10px] font-mono uppercase tracking-wider text-memory">
          ◆ {count} Postgres rows
          {typeof recall === "number" ? ` · 🧠 ${recall} Hindsight recalls` : ""}
        </div>
      )}
...
function AnalyzeCard() {
  ...
  return (
    <InsightShell
      ...
      run={() => m.mutate()}
      busy={m.isPending}
      count={r && !r.empty ? r.memoryCount : null}
      recall={r && !r.empty ? (r.hindsightRecall ?? null) : null}
    >
      {!r && <EmptyHint />}
      ...
      {r && !r.empty && r.analysis && (
        <div className="space-y-3">
          ...
          <Kv
            k="Best platform"
            v={r.analysis.best_platform.name}
            note={r.analysis.best_platform.reason}
          />
          <Kv
            k="Best format"
            v={r.analysis.best_format.name}
            note={r.analysis.best_format.reason}
          />
...
function GapsCard() {
  ...
  return (
    <InsightShell
      title="Content Gaps"
      icon={<Target className="h-4 w-4 text-background" />}
      accent="bg-memory"
      run={() => m.mutate()}
      busy={m.isPending}
      count={r && !r.empty ? r.memoryCount : null}
...
function EmptyHint() {
  return (
    <div className="flex h-full items-center justify-center text-center text-xs text-muted-foreground">
      Press{" "}
      <span className="mx-1 rounded border border-border bg-secondary px-1.5 py-0.5 font-mono">
        Run
      </span>{" "}
      to analyze memory
    </div>
    ...
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      {rows.length === 0 ? (
        <p className="mt-6 text-center text-sm text-muted-foreground">
          No memories yet. Load demo data or add content above.
        </p>
      ) : (
          ...
          {rows.map((r) => (
            <li key={r.id} className="flex items-center justify-between gap-3 py-3">
              ...
              <div className="text-right">
                <div className="font-mono text-sm tabular-nums text-signal">
                  {r.engagement_score}
                </div>
