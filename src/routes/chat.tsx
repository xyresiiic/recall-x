  ...
  return (
    ...
    >
      <header className="mb-4 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Backed by Hindsight Memory · cites real numbers from your history.
          </p>
        </div>
        <button
      ...
      <div className="mb-3 flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-xs">
        <Database
          className={`h-3.5 w-3.5 ${useMemory ? "text-memory" : "text-muted-foreground"}`}
        />
      ...
      <div
        ref={scrollRef}
        className="flex-1 space-y-6 overflow-y-auto rounded-xl border border-border bg-card/60 backdrop-blur px-5 py-6 md:px-8 md:py-8"
      >
...
function Welcome({ useMemory, setInput }: { useMemory: boolean; setInput: (v: string) => void }) {
  const prompts = useMemory
    ...
    : ["What should I post next?", "Best topic for a marketing brand?"];
  return (
    <div className="py-8 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {useMemory
          ? "I'll cite real numbers from your stored memories."
          : "Toggle Memory ON to see the difference."}
      </p>
      ...
      <div className="flex-1 min-w-0 pt-1 text-[15px] leading-[1.7] text-foreground/95">
        <ReactMarkdown
          components={{
            ...
            h1: ({ children }) => (
              <h1 className="mt-5 mb-3 font-display text-xl text-white">{children}</h1>
            ),
            h2: ({ children }) => (
            ...
            ul: ({ children }) => (
              <ul className="my-3 ml-5 list-disc space-y-1.5 marker:text-violet-400">{children}</ul>
            ),
            ol: ({ children }) => (
              <ol className="my-3 ml-5 list-decimal space-y-1.5 marker:text-violet-400">
                {children}
              </ol>
            ),
            ...
            code: ({ children }) => (
              <code className="rounded bg-black/40 px-1.5 py-0.5 font-mono text-[13px] text-cyan-200 border border-white/10">
                {children}
              </code>
            ),
            pre: ({ children }) => (
              <pre className="my-3 overflow-x-auto rounded-lg border border-white/10 bg-black/50 p-4 text-[13px] leading-relaxed">
                {children}
              </pre>
            ),
            blockquote: ({ children }) => (
              <blockquote className="my-3 border-l-2 border-violet-400/60 pl-4 italic text-white/80">
                {children}
              </blockquote>
            ),
